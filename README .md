...
| SQL (MySQL) | 15+ queries — aggregations, `CASE WHEN` logic, `RANK()` window functions |
| Python (Jupyter Notebook, Pandas, Matplotlib) | Statistical summaries, binning (age/experience bands), chart generation |
...
├── hr_mysql_schema_and_data.sql     # Run this FIRST — creates DB + loads 1,000 rows (MySQL Workbench)
├── hr_sql_analysis_mysql.sql        # All 15 analysis queries (run after the file above)
├── hr_python_analysis.ipynb         # Jupyter Notebook — Pandas + Matplotlib analysis and charts
├── hr_python_analysis_charts.png    # Generated chart grid (hiring trend, headcount, attrition)
├── hr_employee.db                   # SQLite copy of the dataset (for quick local testing)
